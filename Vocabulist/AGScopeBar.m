...
@property (weak, readwrite) AGScopeBarGroup * group;
@property (weak, readonly) NSButton * button;
@property (weak, readonly) NSMenuItem * menuItem;
...
@property (readwrite, weak) AGScopeBar * scopeBar;
...
- (id)initWithFrame:(NSRect)frameRect
{
	...
	mBottomBorderColor = SCOPE_BAR_BORDER_COLOR;
...
- (void)resizeSubviewsWithOldSize:(NSSize)oldBoundsSize
{
    ...
    if (oldBoundsSize.width == newSize.width)
        return;
...
- (CGFloat)_widthForPopup:(NSPopUpButton *)popup
{
	...
	if (POPUP_RESIZES) {
		...
		NSPopUpButtonCell * inCell = [popup cell];
		NSPopUpButtonCell * cell = [[NSPopUpButtonCell alloc] initTextCell:inCell.title pullsDown:inCell.pullsDown];
		...
		[popup.cell setMenuItem:[[NSMenuItem alloc] init]];
		[self _updatePopup];
	}
